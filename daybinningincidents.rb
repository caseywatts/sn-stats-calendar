...
CSV_FILE = "incidents"
csvfile = CSV.read(CSV_FILE+".csv", :headers => true, :encoding => "windows-1251:utf-8")
csvfile.each do |inc|
  inc["start_date"] = Date.strptime(inc["opened_at"], "%Y-%m-%d %H:%M:%S")
  inc["end_date"] = Date.strptime(inc["u_resolved"], "%Y-%m-%d %H:%M:%S") unless inc["u_resolved"].empty?
end
...
CSV.open(CSV_FILE+"opened.csv", "w") do |csvoutput|
...
CSV.open(CSV_FILE+"resolved.csv", "w") do |csvoutput|
  csvoutput << ["date", "value"]
  (mindate..maxdate).each do |date|
...
CSV.open(CSV_FILE+"duration.csv", "w") do |csvoutput|
  csvoutput << ["date", "value"]
  (mindate..maxdate).each do |date|
    count = 0
...
CSV.open(CSV_FILE+"timeworked.csv", "w") do |csvoutput|
  csvoutput << ["date", "value"]
  (mindate..maxdate).each do |date|
    count = 0
    selectedrows = csvfile.find_all { |row| row["start_date"] == date }
...
CSV.open(CSV_FILE+"active.csv", "w") do |csvoutput|
  csvoutput << ["date", "value"]
  (mindate..maxdate).each do |date|
    count = 0
    csvfile.each do |row|
...
#CT Tasks Opened
CSV.open("cttasksopened.csv", "w") do |csvoutput|
  csvoutput << ["date", "value"]
  (mindate..maxdate).each do |date|
    count = csvfile["start_date"].count { |d| d == date}
    csvoutput << [date.to_s, count.to_s]
